  ...
  <div class='px-4 py-20 bg-gray-100 sm:px-0'>
    ...
    <div v-else class='grid overflow-hidden gap-4 mx-auto max-w-7xl sm:px-6 lg:px-8 sm:grid-cols-2 md:grid-cols-3 sm:gap-8'>
      <div v-for='category in allCategoriesWithSubcategories' :key='category.id'
        ...
        <div>
          <span :style="'background-color:'+this.$filters.tinycolor(category.color).setAlpha(0.15)" class='inline-flex p-4 rounded-2xl ring-4 ring-white'>
            <img :alt='this.$filters.transString(category?.name)' :src='this.$filters.getFirstMediaUrl(category)' aria-hidden='true' class='w-12 h-12 opacity-60'>
          </span>
        </div>
        <div class='mt-4'>
