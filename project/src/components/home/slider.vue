        ...
        <slide v-for='slide in slides' :key='slide' class='relative w-full shadow-xl sm:overflow-hidden'>
          ...
          <div class='flex relative items-center px-6 w-full h-48 sm:px-10 sm:h-96'>
            <div class='w-1/3'>
              ...
              <router-link v-else-if='slide.salon_id !== null' :to='{name:"Salon",params:{id:slide.salon_id || 0}}' :style="'background-color:'+slide.button_color" class="block px-3 py-3 w-32 rounded-full text-sm text-white text-center my-1 mx-auto">{{ this.$filters.transString(slide.button) }}</router-link>
